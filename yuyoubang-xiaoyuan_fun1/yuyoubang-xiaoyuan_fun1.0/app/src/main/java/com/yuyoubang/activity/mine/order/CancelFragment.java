...
public class CancelFragment extends BaseNetFragment<Order> {


    @Bind(R.id.cancel_order_rv)
    RecyclerView cancelOrderRv;
    @Bind(R.id.cancel_order_sp)
    SwipeRefreshLayout cancelOrderSp;
...
//        testCall.enqueue(this);
    ...
    private void initView() {
        ...
        cancelOrderSp.setColorSchemeColors(getActivity().getResources().getColor(R.color.color_ff9600));
        cancelOrderSp.setOnRefreshListener(new SwipeRefreshLayout.OnRefreshListener() {
            @Override
        ...
        cancelOrderRv.setLayoutManager(layoutManager);
        cancelOrderRv.setAdapter(mAdapter);
        getData("");
    ...
    private void getData(final String cursor) {
        ...
        QLog.d("uid",PreferenceUtils.getsessionId(getActivity()));
        Call<Order> order = mineApi.my_trip_participate_list_cancelled(body);
        order.enqueue(this);
    }
    ...
    protected void processData(Order order) {
        ...
        if (TextUtils.isEmpty(cursor)) {
            ...
            cancelOrderSp.setRefreshing(false);
    ...
    protected int getContentResId() {
        return R.layout.frg_order_cancel;
    }
