...
public class SerachUserAdapter extends MultiBaseAdapter<SearchUser.ResultBean.ListBean> {
    private Context context;
    private TextView tv_find_info_follow;
    private String locationCity = "";

    public SerachUserAdapter(Context context, List<SearchUser.ResultBean.ListBean> listBean, boolean isOpenLoadMore) {
        super(context, listBean, isOpenLoadMore);
        this.context = context;
    }

    @Override
    protected int getViewType(int position, SearchUser.ResultBean.ListBean data) {
        return 1;
    }

    @Override
    protected void convert(ViewHolder holder, final SearchUser.ResultBean.ListBean data, int viewType) {
...
//            location.setText(data.getUser_data().getUser_location());
        ...
        if (data.getIs_like_to_user() == 0) {//未关注他人
                ...
                if (response.body().getError_code() == 0) {
                    for (int i = 0; i < mDatas.size(); i++) {
                        if (mDatas.get(i).getId() == Long.valueOf(uid)) {
                            mDatas.get(i).setIs_like_to_user(1);
                        }
                    }
        ...
        followUserCall.enqueue(new Callback<RemoveUser>() {
            ...
            public void onResponse(Call<RemoveUser> call, Response<RemoveUser> response) {
                ...
                if (body.getError_code() == 0) {
                    for (int i = 0; i < mDatas.size(); i++) {
                        if (mDatas.get(i).getId() == Long.valueOf(uid)) {
                            mDatas.get(i).setIs_like_to_user(0);
                        }
                    }
                    notifyDataSetChanged();
                }
