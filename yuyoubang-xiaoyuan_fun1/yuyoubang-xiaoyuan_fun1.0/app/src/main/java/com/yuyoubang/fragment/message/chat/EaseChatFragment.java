...
import com.yuyoubang.activity.msg.LocationAmapActivity;
...
import com.yuyoubang.fragment.message.chatrow.EaseCustomChatRowProvider;
import com.yuyoubang.fragment.message.photoview.EaseAlertDialog;
import com.yuyoubang.fragment.message.photoview.EaseChatExtendMenu;
import com.yuyoubang.fragment.message.photoview.EaseChatInputMenu;
import com.yuyoubang.fragment.message.photoview.EaseChatMessageList;
import com.yuyoubang.fragment.message.photoview.EaseGroupRemoveListener;
import com.yuyoubang.fragment.message.photoview.EaseVoiceRecorderView;
...
public class EaseChatFragment extends EaseBaseFragment implements EMMessageListener {
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        try {
            user = PreferenceUtils.deSerialization(PreferenceUtils.getPrefString(getActivity(), "user", ""));
            if (user == null) {
                ToastUtils.showShort("账号异常");
                YuYouBangApp.logOut();
                LaunchOperate.openLoginAccActivity(getActivity());
                getActivity().finish();
            }
        } catch (IOException e) {
            e.printStackTrace();
        ...
        if (chatType != EaseConstant.CHATTYPE_GROUP) {
            getUserForInfo(toChatUsername);//获取聊天对象的信息
        }
    ...
    protected void initView() {
        ...
        inputMenu.setChatInputMenuListener(new EaseChatInputMenu.ChatInputMenuListener() {
            ...
            @Override
            public void onBigExpressionClicked(com.yuyoubang.fragment.message.emojicon.EaseEmojicon emojicon) {
        ...
        messageList.setItemClickListener(new EaseChatMessageList.MessageListItemClickListener() {
            ...
            public void onUserAvatarClick(String username) {
                if (chatFragmentHelper != null) {
                    LaunchOperate.openOtherHome(getActivity(), username);
        ...
        public void onClick(int itemId, View view) {
            ...
            switch (itemId) {
                ...
                case ITEM_PICTURE:
                    ...
                    break;
                case ITEM_LOCATION:
                    startActivityForResult(new Intent(getActivity(), LocationAmapActivity.class), REQUEST_CODE_MAP);
//                    Intent intent = new Intent(getActivity(), LocationAmapActivity.class);
//                    startActivity(intent);
    ...
    protected void sendMessage(EMMessage message) {
        ...
        if (TextUtils.isEmpty(user.getUser_name())) {
            message.setAttribute("fromeUserName", "遇游邦Test");
        } else {
            message.setAttribute("fromeUserName", user.getUser_name());
        }
        message.setAttribute("fromeUserPic", user.getProfile_pic_url());
        message.setAttribute("fromeUserId", PreferenceUtils.getsessionId(getActivity()));
        message.setAttribute("toUserName", toUserName);
        message.setAttribute("toUserPic", toUserPic);
        message.setAttribute("toUserId", toChatUsername);
        //send message
    ...
    private void getUserForInfo(String toChatUsername) {
                    ...
                    if (response.body().getResult() != null) {
                        if (response.body().getResult().getUser() != null) {
                            if (response.body().getResult().getUser().size() > 0) {
                                ...
                                toUserPic = response.body().getResult().getUser().get(0).getUser_data().getProfile_pic_url();
                            }
                        }
                    }
                }
