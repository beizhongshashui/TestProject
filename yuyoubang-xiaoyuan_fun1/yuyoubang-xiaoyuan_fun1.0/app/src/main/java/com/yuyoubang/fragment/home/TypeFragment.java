...
public class TypeFragment extends BaseNetFragment<HotBean> {
    ...
    public void SetType(String type) {
        trip_route_type = type;
        getData("", trip_route_type, city);
    }
    ...
    private void initView() {
        ...
        trip_route_type = getArguments().getString("type");
        Log.d("TypeFragment", "trip_route_type=" + trip_route_type);
...
//                getData("", trip_route_type, city);
                if (!TextUtils.isEmpty(homeActivity.city)) {
                    getData("", trip_route_type, homeActivity.city);
                } else {
                    getData("", trip_route_type, city);
                }
...
//                    getData(data.get(data.size() - 1).getId() + "", trip_route_type, city);
                    if (!TextUtils.isEmpty(homeActivity.city)) {
                        getData(data.get(data.size() - 1).getId() + "", trip_route_type, homeActivity.city);
                    } else {
                        getData(data.get(data.size() - 1).getId() + "", trip_route_type, city);
                    }
                }
...
//        typeRecyclerView.addItemDecoration(new ItemDivider(getActivity(),R.drawable.divider_item));
    ...
    private void getData(final String cursor, String trip_route_type, String city) {
        this.city = city;
        HomeApi homeApi = RetrofitManager.getTestRetrofit().create(HomeApi.class);
        FormBody body;
        if (trip_route_type.equals("全部路线类型")) {
...
//            map.put("cursor", cursor);
            ...
            Map<String, String> map = ParamsUtil.getParams();
            map.put("meeting_city", city);
            map.put("trip_route_type", trip_route_type);
            map.put("cursor", cursor);
            FormBody.Builder builder = ParamsUtil.filterParams(map, new FormBody.Builder());
            body = builder.build();
        }
        ...
        Call<HotBean> login = homeApi.trip_trpe_show(body);
