...
public class FindMoreLikeUserAdapter extends MultiBaseAdapter<MoreLikeList.ResultBean.ListBean> {
    private Context context;
    private TextView tv_find_info_follow;
    private String flag;
    private String locationCity = "";

    public FindMoreLikeUserAdapter(Context context, List<MoreLikeList.ResultBean.ListBean> listBean, boolean isOpenLoadMore, String flag) {
        ...
        tv_find_info_follow.setOnClickListener(new OnClickListener() {
            ...
            protected void clickOperate() {//是否关注
                if (data.getOp_data().getIs_like_to_user() == 0) {//未关注他人
                    followUser(String.valueOf(data.getUid()));
                } else {
                    cancelCare(String.valueOf(data.getUid()));
                }
            }
        });
            ...
            public void onResponse(Call<FollowUser> call, Response<FollowUser> response) {
                if (response.body().getError_code() == 0) {
                    for (int i = 0; i < mDatas.size(); i++) {
                        if (mDatas.get(i).getUid() == Long.valueOf(uid)) {
